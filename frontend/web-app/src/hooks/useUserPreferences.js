...
const useUserPreferences = (isAuthenticatedParam = null) => {
    ...
    const loadPreferences = async () => {
      ...
      try {
        ...
        if (preferencesData && typeof preferencesData === 'object' && Object.keys(preferencesData).length > 0) {
          ...
          // Ensure setup_complete is properly converted to boolean
          let setupComplete = false;
          if (preferencesData.setup_complete !== undefined) {
            setupComplete = preferencesData.setup_complete === true || 
                           preferencesData.setup_complete === 'true' || 
  ...
  const updatePreferences = async (newPreferences) => {
    ...
    // Ensure setup_complete is properly converted to boolean
    if (updatedPreferences.setup_complete !== undefined) {
      updatedPreferences.setup_complete = updatedPreferences.setup_complete === true || 
                                         updatedPreferences.setup_complete === 'true' || 
    ...
    try {
      ...
      if (response && response.preferences) {
        ...
        // Ensure setup_complete is properly converted to boolean
        if (savedPreferences.setup_complete !== undefined) {
          savedPreferences.setup_complete = savedPreferences.setup_complete === true || 
                                           savedPreferences.setup_complete === 'true' || 
                                           savedPreferences.setup_complete === 1;
        }
      }
